...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def go(count: Int, cur: Int, prev: Int): Int = 
      if (count == n)
        cur
      else 
        go(count + 1, cur + prev, cur)
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def isOrdered(elem: A, rest: Array[A]): Boolean = {
      if (rest.isEmpty)
        true
      else if (!gt(elem, rest.head))
        false
      else
        isOrdered(rest.head, rest.tail)
    }

    if (as.isEmpty || as.length == 1)
      false
    else
      isOrdered(as.head, as.tail)
  }
